...
/**
 * The published Umbra native prover declares a consumer ProGuard file but does
 * not ship it. Its JNI entry points use the `com.moproffi` class names, so this
 * is the only app-owned keep rule required beyond the rules supplied by Expo,
 * React Native, Reanimated and Worklets themselves.
 */
...
export default ({ config }: ConfigContext): ExpoConfig => ({
  ...
  // Expo's native fingerprint is the OTA compatibility authority. The two
  // deterministic-install exceptions are documented narrowly in .fingerprintignore.
  ...
  plugins: [
    ...
    // These two Gradle properties are not exposed by expo-build-properties.
    // Keep them in a small plugin because `android/` is regenerated by Prebuild.
