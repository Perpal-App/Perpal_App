...
4. Funding is one resumable operation: M deposits collateral and a user-chosen SOL fee reserve into Umbra, Umbra's relayer claims them into T, then Perpal initializes and funds the selected provider internally.
...
- `workers/gateway/`: authenticated bounded Solana RPC and public market-data routes.
